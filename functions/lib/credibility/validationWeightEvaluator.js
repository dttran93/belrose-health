...
exports.computeRecordScoreForHash = computeRecordScoreForHash;
exports.evaluatePendingDisputes = evaluatePendingDisputes;
 ...
 */
async function computeRecordScoreForHash(db, recordId, recordHash) {
    ...
    const eventsSnap = await db
        ...
        .get();
    const contributions = eventsSnap.docs.map(eventDoc => {
        const data = eventDoc.data();
        return {
            eventType: data.eventType,
            contributionDelta: data.contributionDelta ?? 0,
        };
    });
    return (0, _shared_1.aggregateRecordScore)(contributions).score;
}
/**
 ...
 */
async function evaluatePendingDisputes(db) {
    const cutoff = firestore_1.Timestamp.fromMillis(Date.now() - constants_1.EVALUATION_WINDOW_DAYS * 24 * 60 * 60 * 1000);
    ...
    for (const disputeDoc of pendingSnap.docs) {
        ...
        const currentScoreForHash = await computeRecordScoreForHash(db, dispute.recordId, dispute.recordHash);
        if (currentScoreForHash === null) {
            console.warn(`⚠️ Skipping ValidationWeight evaluation for dispute ${disputeDoc.id} — record ${dispute.recordId} not found`);
            continue;
        }
    ...
    console.log(`📊 ValidationWeight: ${evaluated} dispute(s) resolved, ${stillSilent} left pending (no score movement yet)`);
    return { evaluated };
}
//# sourceMappingURL=validationWeightEvaluator.js.map