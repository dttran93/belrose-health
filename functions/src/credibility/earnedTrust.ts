...
import { INITIAL_SCORE } from '../_shared';
import {
  DISPUTE_SEVERITY_WEIGHTS,
  CULPABILITY_WEIGHTS,
  DEFAULT_CREDENTIAL_FLOOR,
  UNACCEPTED_RECORDS_PENALTY_CONSTANT,
} from './constants';
import type { EarnedTrustInputs, EarnedTrustResult, RecordSummary, UserForCredibility } from './types';
 ...
 */
export async function fetchEarnedTrustInputs(db: Firestore): Promise<EarnedTrustInputs> {
  const [usersSnap, verificationsSnap, disputesSnap, unacceptedFlagsSnap, subjectHistorySnap] =
    await Promise.all([
  ...
  const users: UserForCredibility[] = usersSnap.docs.map(userDoc => {
    const data = userDoc.data();
    return {
      uid: userDoc.id,
      credentialFloor: (data.credentialFloor as number | undefined) ?? DEFAULT_CREDENTIAL_FLOOR,
      priorScore: (data.credibility?.score as number | undefined) ?? null,
    };
  });
  ...
  for (let i = 0; i < recordIds.length; i += GET_ALL_CHUNK_SIZE) {
    ...
    snaps.forEach(snap => {
      ...
      records.set(snap.id, {
        ...
        credibilityScore: (data.credibility?.score as number | undefined) ?? INITIAL_SCORE,
      });
    });
  }

  const unacceptedFlagCounts = new Map<string, number>();
  unacceptedFlagsSnap.docs.forEach(d => {
    const subjectId = d.data().subjectId as string;
  ...
  const everAnchoredRecordIds = new Map<string, Set<string>>(); // subjectId -> Set<recordId>
  subjectHistorySnap.docs.forEach(d => {
    const data = d.data();
    if (data.action !== 'anchored' && data.action !== 'anchored_as_controller') return;
    const set = everAnchoredRecordIds.get(data.subjectId) ?? new Set<string>();
    set.add(data.recordId);
    everAnchoredRecordIds.set(data.subjectId, set);
  });
  const everAnchoredRecordCounts = new Map(
    [...everAnchoredRecordIds].map(([subjectId, set]) => [subjectId, set.size])
  );
 ...
 */
export function computeEarnedTrust(inputs: EarnedTrustInputs): Map<string, EarnedTrustResult> {
  const verificationsByUser = new Map<string, VerificationDoc[]>();
  ...
  const disputesByUser = new Map<string, DisputeDoc[]>();
  const disputesByRecordHash = new Map<string, DisputeDoc[]>();
  ...
  const results = new Map<string, EarnedTrustResult>();
  ...
  for (const user of inputs.users) {
    ...
    const avgRecordCredibility = average(
      currentHashVerifications.map(v => inputs.records.get(v.recordId)!.credibilityScore)
    );
    ...
    const disputeAccuracy = average(
      userDisputes.map(d => DISPUTE_SEVERITY_WEIGHTS[d.severity] * d.validationWeight)
    );
    ...
    const culpabilityDisputes: DisputeDoc[] = [];
    for (const v of currentHashVerifications) {
      for (const d of disputesByRecordHash.get(v.recordHash) ?? []) {
        if (seenDisputeIds.has(d.id)) continue;
        seenDisputeIds.add(d.id);
        culpabilityDisputes.push(d);
      }
    }
    const culpabilityPenalty =
      culpabilityDisputes.length === 0
        ? 0
        : (average(culpabilityDisputes.map(d => CULPABILITY_WEIGHTS[d.culpability] * d.validationWeight)) ?? 0);
    ...
    const unacceptedRecordsPenalty =
      everAnchoredCount === 0
        ? 0
        : (flaggedCount / everAnchoredCount) * UNACCEPTED_RECORDS_PENALTY_CONSTANT;

    const earnedTrust = Math.max(
      user.credentialFloor,
      (avgRecordCredibility ?? 0) + (disputeAccuracy ?? 0) - culpabilityPenalty - unacceptedRecordsPenalty
    );
